...
<section id="sources" class="scroll-mt-24">
  <h2 class="mb-4 font-serif text-2xl text-fg">Source directory</h2>
  <p class="text-sm text-fg-2 mb-3">{entries.length} catalogued feeds. {sourceAudit.sources.length} have review records; the remaining reviews are pending. Each record states its evidence limits. A feed is not necessarily a separate news organization. Editorial assessments and fetch status are separate.</p>
  <p class="text-sm text-fg-2 mb-4">Political viewpoint and state funding alone do not justify exclusion. Exclusion requires substantiated repeated fabrication, deceptive attribution or persistently unusable sourcing. Historical articles remain available.</p>
  <p class="text-sm text-fg-2 mb-4">
    <strong class="text-fg-2">How these were produced.</strong> The cited pages and article samples were fetched and retained; the summaries below were drafted by a model from those fetches, not written by a human editor. Every sample reads a publisher excerpt, not the full article, so none of this independently verifies a published claim. Unless a profile says otherwise it was retained with limitations — the decision line appears only where the outcome differs.
  </p>
  <label for="source-search" class="label mb-2 block">Search sources, regions or reviewed ownership</label>
  <input id="source-search" type="search" bind:value={search} class="field mb-2" />
  <p role="status" class="text-xs text-fg-2 mb-4">{filtered.length} matching feeds</p>
  ...
  {#if sources.length}
    <p class="text-xs text-fg-2 mb-4">{sources.filter(s => s.enabled).length} enabled · {sources.filter(s => sourceHealth(s,clock.now) === 'recently fetched').length} successfully fetched within 60 minutes</p>
  {/if}
  <div class="space-y-2">
    {#each filtered as s (s.id)}
      ...
      <details id="source-{s.id}" open={selected === s.id} class="scroll-mt-24 rounded-xl border border-line bg-panel/40">
        <summary class="min-h-11 cursor-pointer p-3 text-sm text-fg break-words">
          <span class="font-medium">{s.name}</span>
          <span class="text-fg-2"> · {s.region} · {s.lang}</span>
        </summary>
        <div class="px-3 pb-4 text-sm text-fg-2 space-y-3 break-words">
          ...
          {#if review && publisher}
            {#if review.decision !== 'retain with limitations'}<p><strong>Editorial decision:</strong> {review.decision}. Reviewed {review.reviewedAt}.</p>{:else}<p class="text-fg-2">Reviewed {review.reviewedAt}.</p>{/if}
            {#if review.rationale}<p>{review.rationale}</p>{/if}
            <dl class="space-y-2">
              <div><dt class="font-medium text-fg">Publisher / ownership</dt><dd>{publisher.name}. {publisher.ownership}</dd></div>
              <div><dt class="font-medium text-fg">Funding</dt><dd>{publisher.funding}</dd></div>
              <div><dt class="font-medium text-fg">Editorial purpose and coverage</dt><dd>{publisher.purpose} Coverage: {publisher.coverage}.</dd></div>
              <div><dt class="font-medium text-fg">Sourcing</dt><dd>{publisher.sourcing}</dd></div>
              <div><dt class="font-medium text-fg">Corrections</dt><dd>{publisher.corrections}</dd></div>
            </dl>
            {#each publisher.limitations as limitation}<p class="text-fg-2">{limitation}</p>{/each}
            <h3 class="font-medium text-fg">Publisher evidence</h3>
            <ul class="list-disc pl-5">
              {#each publisher.evidence as e}<li><a class="link inline-flex min-h-11 items-center break-all" href={e.url} target="_blank" rel="noopener noreferrer">{e.title}</a> <span class="text-fg-2">cited in the {e.accessedAt} review</span></li>{/each}
            </ul>
            <details ontoggle={loadSamples}>
              <summary class="min-h-11 cursor-pointer py-3">Article sample ({review.sampleCount})</summary>
              {#if review.sampleLimitations}<p class="text-fg-2 mb-3">{review.sampleLimitations}</p>{/if}
              {#if samplesFailed}
              ...
              {:else if samples?.[s.id]}
                <ol class="list-decimal pl-5 space-y-3">
                  {#each samples[s.id] as a}<li><a class="link inline-flex min-h-11 items-center" href={a.url} target="_blank" rel="noopener noreferrer" dir="auto">{a.title}</a><p>{a.observation}</p><p class="text-xs text-fg-2">{a.access} · reviewed {a.reviewedAt}{a.publishedAt ? ` · published ${a.publishedAt.slice(0,10)}` : ''}</p></li>{/each}
                </ol>
              {:else}
                <p role="status" class="text-fg-2">Loading {review.sampleCount} samples…</p>
              {/if}
            </details>
          {:else}
